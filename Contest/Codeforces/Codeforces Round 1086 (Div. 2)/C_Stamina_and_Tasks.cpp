...
*/
vector<double> dp;
double mm(vector<pair<double,double>> &v,int index) {
    if(v.size()==index) {
        return 0.0;
    }
    if(dp[index]!=-1.0) {
    ...
    double include=v[index].first+(1.0-(v[index].second)/100.0)*mm(v,index+1);
    double exclude=mm(v,index+1);
...
void solve()
{
    ...
    int n;
    cin>>n;
    vector<pair<double,double>> v(n,{0,0});
    f(i,0,n) {
        double x,y;
        cin>>x>>y;
        v[i]={x,y};
