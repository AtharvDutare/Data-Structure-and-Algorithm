...
void dfs(int node,Graph& gp,vector<int>&visited,int cnt,int &ans) {
    
    //cout<<"atharv"<<endl;
    visited[node]=1;
    for(auto &x:gp[node]) {
        if(!visited[x]) {
            dfs(x,gp,visited,cnt+1,ans);
        }
    }
}
...
bool bfs(int node,int &c0,int &c1) {
    queue<int> q;
    q.push(node);
    visited[node]=0;
    c0++;
    ...
    while(!q.empty()) {
        ...
        for(int &neig:gp[s]) {
            ...
            else if(visited[neig]==visited[s]) {
                ans=false;
            }
        }
    }
