...
void solve()
{
    ...
    int64_t sum=accumulate(all(arr),0LL);
    
    vector<pair<int64_t,int64_t>> track(n,{0,0});
    f(i,0,n) {
        track[i]={arr[i],0};
    }
    pair<int64_t,int64_t> globalTrack={0,-1};
    ...
    f(i,0,q){
        ...
        if(t==1) {
            ...
            if(globalTrack.second>track[j].second) {
                sum=sum-globalTrack.first+x;
            }
            else {
                sum=sum-track[j].first+x;
            }
