...
/* clang-format on 
    ...
    to optimize the code distinct integer is maybe the key 
    we cant sort 
...
void solve()
{
    ...
    // }
    // int ans=0;
    // int maxVal=(2*n)*(2*n-1);
    // for(int i=1;i<=n;i++) {
    //     int ele=arr[i];
    //     if(ele<=maxVal) {
    //        for(int aj=1;aj<=(maxVal/ele);aj++) {
    //             if(((aj*ele-i))>i&&((aj*ele-i)==mp[aj])) {
    //                 ans++;
    //             }
    //        }
    //     }
