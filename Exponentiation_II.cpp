...
/* 
    ...
    Thoughts:
        ...
        we know the fermet therom

        a^P-1/p ==1 if(A and P is co-prime)

        so if we do b^c mod (p) and then 

*/

ll power_it(ll a,ll b,ll m) {
    ll ans=1;
    while(b>0) {
        if(b&1) ans=(ans*a)%m;
        a=(a*a)%m;
...
void solve()
{
    ll a,b,c;
    cin>>a>>b>>c;
    ll k=power_it(b,c,MOD-1);
    ll ans=power_it(a,k,MOD);
    cout<<ans<<endl;
}
